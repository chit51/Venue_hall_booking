...
<script>
  $(document).ready(function(){
    ...
    // Indian Mobile Number Validation in JavaScript
    $('#contact').on('input', function() {
      var contact = $(this).val();
      var contactPattern = /^(?:\+91|91)?[6-9][0-9]{9}$/;
    ...
    $('#register-frm').submit(function(e){
      ...
      if(!namePattern.test(firstname)) {
        el.addClass('alert-danger').text('First name should contain only letters.');
        _this.prepend(el);
        el.show('slow');
        return false;
      }
      ...
      if(middlename && !namePattern.test(middlename)) {
        el.addClass('alert-danger').text('Middle name should contain only letters.');
        _this.prepend(el);
        el.show('slow');
        return false;
      }
      ...
      if(!namePattern.test(lastname)) {
        el.addClass('alert-danger').text('Last name should contain only letters.');
        _this.prepend(el);
        el.show('slow');
        return false;
      }
      ...
      if (!contactPattern.test(contact)) {
        el.addClass('alert-danger').text('Invalid mobile number.');
        _this.prepend(el);
        el.show('slow');
        return false;
      }
